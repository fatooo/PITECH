...
ADC_INIT	PROC
	...
	LDR R1, =PORTE_DEN
	LDR R0, [R1]
	BIC R0, R0, #0x06 ; clear bit 3 to disable analog on PE3 AND PE2
	STR R0, [R1]
	; Enable analog on PE3
	LDR R1, =PORTE_AMSEL
	LDR R0, [R1]
	ORR R0, R0, #0x06 ; set bit 3 to enable analog on PE3
	STR R0, [R1]
	...
	LDR R1, =ADC0_SSMUX3
	LDR R0, [R1]
	;BIC R0, R0, #0x000F ; clear bits 3:0 to select AIN2
	ORR R0, R0, #0x0002
	BIC R0, R0, #0x000D
	STR R0, [R1]
