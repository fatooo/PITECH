...
SPI_INIT	LDR 		R1,=SYSCTL_RCGCSSI		; Initalize Clock
			...
			LDR			R1,=SSI0_CC				;Clock Configuration
			LDR			R0,[R1]
			BIC			R0,R0,#0x0F				;Choose SYSTEM CLK
			STR			R0,[R1]
			
			LDR 		R1,=SSI0_CPSR			;Enable Clock Prescale
			LDR 		R0,[R1]
			ORR 		R0,R0,#0x02				;80MHz to 40MHz  
			STR			R0,[R1]
	
			LDR 		R1,=SSI0_CR0			;Control 0
			LDR 		R0,[R1]
			ORR			R0,R0,#0x900			;40MHz to 4MHz
			ORR 		R0,R0,#0x07				;8 bit Freescale mode
			STR			R0,[R1]
